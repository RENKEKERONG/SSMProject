...
function chooseProvince() {
    ...
    obj.onclick = function () {
        ...
        sele[index].onclick = city(Array[1]);
    }
}
...
function chooseCity() {
    ...
    obj.onclick = function () {
        ...
        sele[index].onclick = district(Array[1]);
    }
}
...
function chooseDistrict() {
    try {
        ...
        obj.onclick = function () {
            ...
            sele[index].onclick = getDistrictId(Array[1]);
        }
    } catch (e) {
...
function searchCompany() {
    //为了保留其他公司的信息，则当公司名称为三个字的时候，根据他输入的公司名称，去数据库进行相应的模糊查询
    var company = $("#company").val();
    var reg = new RegExp("[\\u4E00-\\u9FFF]+", "g");
    //规则是,若是中文，则三个中文，则将输入框的信息进行模糊查询，带出公司的信息,若是英文，则六个字母带出输入框的信息进行模糊查询
    if (/^[\u4e00-\u9fa5]+$/i.test(company)) {//是中文
        var companyLength = $("#company").val().length;
        //防止公司长度为3后，公司信息带出来了，当输入框的信息重新输入时，清空下拉框的信息
        if (companyLength == 2) {
            $("#car").children().remove();
        } else if (companyLength > 3) {
            var param = new Object();
            param.company = company;
            $.pxAjax("/user/personCenter/searchCompany", param, function (data) {
                $("#car").children().remove();
                if (data.success) {
                    for (var i = 0; i < data.data.length; i++) {
                        var typeId = data.data[i].id;
                        $("#car").append("<option id=" + typeId + " >" + data.data[i].company + "</option>");
                    }
                } else {
                    $.toast("提示", data.msg, "warning");
                }
        ...
        //如果长度大于三可能该公司已被选中,则带着输入框的值去数据库找该公司，进行页面数据赋值
        // else if(companyLength>4){
        //     //带着公司的名字去数据库搜索该公司的信息
        //     var param = new Object();
        //     var map = new Object();
        //     map.company = company;
        ...
        //                     var HTML = "<div>"+addName+"&nbsp;&nbsp;&nbsp<a " +
        //                         "style='color:red;cursor:pointer'></a></div>";
        //                     //生成HTML
        //                     var elem_li = document.createElement('div'); // 生成一个 li元素
        //                     elem_li.id="del"+i;
        //                     elem_li.innerHTML = HTML; // 设置元素的内容
        ...
        //                 }
        //                 //将上传营业执照的拿掉
        //                 $("#srcLicense").removeAttr("onclick");
        //                     $("#srcLicense").addClass('img');
        //             }
        //         } else {
        //             $.toast("提示", data.msg, "warning");
        //         }
    ...
    $.pxAjax("/user/personCenter/accurateSearchBycom", param, function (data) {
        if (data.success) {
            if (data.data.length > 0) {
                ...
                $("#district").val(data.area);
                //将行业类型贴到页面上
                document.getElementById('company_type').textContent = '';
                for (var i = 0; i < data.industryList.length; i++) {
                    var addName = data.industryList[i].industryName1 + "-" + data.industryList[i].industryName2 + "-" + data.industryList[i].industryName3;
                    var HTML = "<div>" + addName + "&nbsp;&nbsp;&nbsp<a " +
                        "style='color:red;cursor:pointer'></a></div>";
...
function refusetCompany(company) {
    //带着公司的名字去数据库搜索该公司的信息
    var param = new Object();
    var map = new Object();
    map.company = company;
...
function saverefuseCompany() {
    ...
    if (flag) {
        ...
        if(status==1){
            //个人加入公司被拒
            $.pxAjax("/user/personCenter/personReject", param, function (result) {
                if (result.success) {
                    location.href = "/user/personCenter/indexing";
                } else {
                    $.toast("提示", result.msg, "warning");
            ...
            //公司审核被拒
            $.pxAjax("/user/personCenter/reAuthentication", param, function (result) {
                if (result.success) {
                    location.href = "/user/personCenter/indexing";
                } else {
                    $.toast("提示", result.msg, "warning");
